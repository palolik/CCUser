...
const Echat = ({ selectedTaskId }) => {
  ...
  useEffect(() => {
    ...
    newSocket.onmessage = (event) => {
      ...
      setMessages((prev) => {
        if (!prev.some((msg) => msg.time === newMessage.time && msg.text === newMessage.text)) {
    ...
    try {
      ...
      isNearBottomRef.current = true;
       if (hasFiles) {
              ...
              const response = await fetch(`${base_url}/addempchat/files`, {
                method: 'POST',
                body: formData,
              });
          ...
          <div
            ...
            >
               {messages.map((msg, index) => {
                const isClient = msg.sender === 'emp';
                return (
            <div
              key={index}
              className={`chat ${msg.sender === 'emp' ? 'chat-end' : 'chat-start'}`}
            >
