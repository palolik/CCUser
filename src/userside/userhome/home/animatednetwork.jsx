...
const AnimatedNetworkBackground = () => {
  ...
  const particleCount = Math.min(
    ...
    Math.max(60, Math.floor(area / 10000))
  );
...
vy: (Math.random() - 0.5) * 0.9,
    ...
    const draw = () => {
     ...
     const connectDistance = window.innerWidth < 768 ? 110 : 155;
const mouseRadius = window.innerWidth < 768 ? 95 : 140;
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < connectDistance) {
            ...
            ctx.strokeStyle = `rgba(96, 165, 250, ${opacity * 0.22})`;
            ctx.lineWidth = 1;
            ctx.stroke();
          }
        }
